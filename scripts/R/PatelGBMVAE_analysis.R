...
model.dir <- "results/gbm/Train1000gGBMVAE/GBMVAE_Final"
max.k <- 12
force.k <- 7
features.end.col <- -1
label.col <- "tumor_ids"
...
run.tcga.projection <- FALSE

#### Load Data -----------------------------------------------------------------
# Load and center TCGA GBM data
tcga.gbm <- readRDS("data/TCGA/original/gbm.logtpm.rds")
exprs(tcga.gbm) <- t(scale(t(exprs(tcga.gbm)), scale=FALSE))

# Load single-cell data (original and processed)
sc.gbm <- readRDS("data/GSE57872_GBM/original/gbm.rds")
sc.gbm <- sc.gbm[,sc.gbm$sample_type == "primary_GBM"]

dataset <- readRDS(dataset.file)
dataset <- dataset[,dataset$sample_type == "primary_GBM"]
dataset$cell_origin <- droplevels(dataset$cell_origin)

latent.space.data <- loadLatentSpace(model.dir, features.end.col, label.col)
latent.space <- latent.space.data$latent.space
...
#### Get Marker Genes ----------------------------------------------------------
if (run.marker.genes) {
  marker.genes <- getMarkerGenes(sc.gbm, clusters,
                                 base.dir=model.dir,
                                 output.dir=paste(
                                   "marker_genes_k", 
  ...
  annotations <- data.frame(Cluster = sc.gbm$clusters,
                            TumorID = sc.gbm$cell_origin)
  rownames(annotations) <- colnames(sc.gbm)
  cluster.colors <- brewer.pal(
    length(unique(sc.gbm$clusters)), "Set3")
  names(cluster.colors) <- unique(sc.gbm$clusters)
  
  tumorID.colors <- brewer.pal(
    length(unique(sc.gbm$cell_origin)), "Spectral")
  names(tumorID.colors) <- unique(sc.gbm$cell_origin)
  ...
  png(filename=paste(model.dir, "/marker_genes_k", length(unique(clusters)),
      ...
      pointsize=4, res=300)
  pheatmap(exprs(sc.gbm[all.marker.genes,]),
           
           color=colorRampPalette(rev(brewer.pal(11, "RdBu")))(100),
           
           cluster_cols=clust.results$results[[force.k]]$consensusTree,
