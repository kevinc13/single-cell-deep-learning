...
class TrainMelanomaVAE(CrossValidationExperiment, HyperoptExperiment):
    def __init__(self, n_genes, debug=False):
        super(TrainMelanomaVAE, self).__init__(debug=debug)
        ...
        self.logger.info(
            "Loaded {}g melanoma dataset".format(
                self.input_size))
    ...
    def load_data(self):
        filepath = "{}/data/GSE72056_Melanoma/processed/" \
                   "malignant_cells.{}g.txt".format(
    ...
    def get_model_config(self, case_config):
        model_name = "{}_MelanomaVAE".format(self.case_counter)
...
class Train1000gMelanomaVAE(TrainMelanomaVAE):
    def __init__(self, debug=False):
        super(Train1000gMelanomaVAE, self).__init__(1000, debug=debug)
