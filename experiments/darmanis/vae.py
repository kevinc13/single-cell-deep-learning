...
class TrainDarmanisGBMVAE(CrossValidationExperiment, HyperoptExperiment):
    def __init__(self, n_genes, debug=False):
        super(TrainDarmanisGBMVAE, self).__init__(debug=debug)
        ...
        cell_ids, locations, tumor_ids, features = self.load_data()
        self.datasets = kfold(
            features, [cell_ids, tumor_ids, locations], n_folds=N_FOLDS)
        self.logger.info(
    ...
    def load_data(self):
        filepath = "{}/data/GSE84465_GBM/processed/gbm.{}g.txt"\
        ...
        locations = df.iloc[:, 0].values
        tumor_ids = df.iloc[:, 1].values
        cell_ids = df.index.values

        return cell_ids, locations, tumor_ids, features_scaled

    def get_model_config(self, case_config):
        model_name = "{}_DarmanisGBMVAE".format(self.case_counter)
    ...
    def train_final_vae(self, model_config):
        ...
        results = np.hstack((
            ...
            np.expand_dims(full_dataset.sample_data[1], axis=2)
        ))

        header = ["cell_id"]
        for l in range(1, model_config["latent_size"] + 1):
            header.append("dim{}".format(l))
        header.append("tumor_id")
...
class Train1000gDarmanisGBMVAE(TrainDarmanisGBMVAE):
    def __init__(self, debug=False):
        super(Train1000gDarmanisGBMVAE, self).__init__(1000, debug=debug)
