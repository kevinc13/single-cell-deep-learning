...
class Experiment(BaseExperiment):
    def __init__(self, debug=False):
        ...
        self.experiment_name = "train_usokin-1000g-scaled-3L-adam-vae"
        if self.debug:
    ...
    def load_data(self):
        df = np.array(self.read_data_table(
            "data/Usokin/processed/mouse_neuronal_cells.1000g.scaled.txt"))
        features = df[1:, 1:-2]
    ...
    def generate_search_space(self):
        return {
            "encoder_layer_sizes": [
                hp.choice("layer1", list(np.arange(600, 951, step=50))),
                hp.choice("layer2", list(np.arange(400, 551, step=50))),
                hp.choice("layer3", list(np.arange(150, 351, step=50)))
            ],
            "latent_size": hp.choice(
                "latent_size",
                list(np.arange(20, 101, step=20))),
